...
class AnomalySubBatch(object):
    def __init__(self, dota, index):
        ...
        # negative case
        if self.end >= dota.metadata[key]['anomaly_start'] and self.begin <= dota.metadata[key]['anomaly_end']:
            self.label = 1
            self.a_start = max(0, dota.metadata[key]['anomaly_start'] - self.begin)
            self.a_end = min(dota.metadata[key]['anomaly_end'] - self.begin, self.end - self.begin)
        else:
            self.label = -1
            self.a_start = -1
            self.a_end = -1
...
class Dota(Dataset):
    def __init__(self, root_path, phase, transforms={'image': None}, VCL=None, vertical_flip_prob=0., horizontal_flip_prob=0.):
    ...
    def get_data_list(self):
        list_file = os.path.join(self.root_path, 'metadata', 'metadata_{}.json'.format(self.phase))
    ...
    def _filter_wrong_metadata(self):
        ...
        for index in range(len(self.metadata)):
            ...
            if count_ann != count_files or count_files != count_meta or count_ann != count_meta:
    ...
    def _get_random_subbatch(self, count):
        if self.VCL is None:
            ...
            # if video is smaller than VCL, return full video
            if count <= self.VCL:
                return 0, count
            max_ = count - self.VCL
    ...
    def read_video(self, index, sub_batch):
        ...
        for index, name in enumerate(frames[sub_batch.begin:sub_batch.end]):
            ...
            if label == 1 or index < sub_batch.a_start or index > sub_batch.a_end:
    ...
    def gather_info(self, index, sub_batch, video_len_orig):
        ann = self.annotations[index]
        label = sub_batch.label
        return np.array([video_len_orig, self.keys.index(ann['video_name']), sub_batch.a_start,
                        sub_batch.a_end, label, sub_batch.a_start, sub_batch.a_end, ann['accident_id'],
                        int(ann['ego_involve']), int(ann['night']), int(has_objects(ann))]).astype('float')
...
def setup_dota(Dota, cfg, num_workers=-1, VCL=None, phase=None):
    ...
    transform_dict_test = {'image': transforms.Compose([pad_frames(cfg.input_shape), transforms.Lambda(lambda x: torch.tensor(x)),
    ...
    transform_dict_train = {'image': transforms.Compose([pad_frames(cfg.input_shape), transforms.Lambda(lambda x: torch.tensor(x)),
                                                         transforms.Lambda(lambda x: x.permute(0, 3, 1, 2)), T.AugMix(),
                                                         transforms.Lambda(lambda x: x / 255.0), transforms.Normalize(params['mean'], params['std'])])}
    transform_dict_to_play = {'image': None}
    traindata_loader, testdata_loader = None, None
    if phase == 'train':
        train_data = Dota(cfg.data_path, 'train', transforms=transform_dict_train, VCL=VCL, vertical_flip_prob=vertical_flip_prob, horizontal_flip_prob=horizontal_flip_prob)
        ...
        test_data = Dota(cfg.data_path, 'val', transforms=transform_dict_test)
        testdata_loader = DataLoader(dataset=test_data, batch_size=1, shuffle=False, drop_last=True, num_workers=num_workers, pin_memory=True)
        print("Total testing set: {}".format(len(test_data)))
    elif phase == 'play':
        test_data = Dota(cfg.data_path, 'val', transforms=transform_dict_to_play)
        testdata_loader = DataLoader(dataset=test_data, batch_size=1, shuffle=False, drop_last=True, num_workers=num_workers, pin_memory=True)
        print("Total testing set: {}".format(len(test_data)))
